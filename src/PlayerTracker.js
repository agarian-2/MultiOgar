...
class PlayerTracker {
    ...
    setSkin(skin) {
        ...
        writer = new BinaryWriter();
        writer.writeStringZeroUtf8("%" + skin);
        this._skinUtf8protocol11 = writer.toBuffer();
    }
    getScale() {
    ...
    checkConnection() {
        if (this.socket.isConnected === false) {
            ...
            if (pt < 0) return;
            let dt = (this.gameServer.stepDateTime - this.socket.closeTime) / 1000;
            if (!this.cells.length || dt >= pt) {
                this.cells = [];
                for (let i = 0; i < this.cells.length; i++) this.gameServer.removeNode(this.cells[i]);
                this.isRemoved = true;
                return;
            }
    ...
    updateTick() {
        ...
        let viewBaseX = config.serverViewBaseX,
            viewBaseY = config.serverViewBaseY;
        if (this.isSpectating) {
            viewBaseX = 2225;
            viewBaseY = 1275;
        }
        let scale = Math.max(this.getScale(), config.serverMinScale),
    ...
    getSpecTarget() {
        ...
        if (target == null || target.isRemoved || !target.cells.length || !target.socket.isConnected) {
            target = null;
            return target;
        }
        return target;
