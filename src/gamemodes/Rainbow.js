...
Rainbow.prototype.changeColor = function (node) {
	if (typeof node.rainbow == 'undefined') node.rainbow = Math.floor(Math.random() * this.colors.length);
	if (node.rainbow >= this.colorLength) node.rainbow = 0;
...
Rainbow.prototype.onTick = function (gameServer) {
	for (var i in gameServer.nodes.all) {
