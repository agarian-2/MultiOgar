...
class GameServer {
    constructor() {
        ...
        this.config = {
            ...
            serverChat: 1,
            serverChatAscii: 1,
            serverName: "My MultiOgar",
            serverWelcome1: "Welcome to MultiOgar!",
            serverWelcome2: "",
            clientBind: "",
            filterBadWords: 1,
    ...
    socketEvent(ws) {
        ...
        let PlayerTracker = require("./PlayerTracker");
        ws.playerTracker = new PlayerTracker(this, ws);
        let PacketHandler = require("./PacketHandler");
        ws.packetHandler = new PacketHandler(this, ws);
        let PlayerCommand = require("./modules/PlayerCommand");
        ...
        ws.on("close", () => {
            ...
            if (self.config.playerGrayDisconnect) {
                let gray = Math.min(255, (ws.playerTracker.color.r * .2125 +
                ...
                ws.playerTracker.cells.forEach(cell => {
                    cell.color = color;
                });
            }
        });
    ...
    broadcastMSG(msg) { // Rename to broadcastMessage later
        for (let i = 0; i < this.clients.length; i++) this.clients[i].sendPacket(new Packet.ChatMessage(null, msg));
    }
    timerLoop() {
    ...
    updateDecay() {
        for (let i = 0; i < this.clients.length; i++) {
            ...
            for (let j = 0; j < client.cells.length; j++) {
                ...
                let rate = client.recMode || client.frozen ? 0 : this.config.playerDecayRate,
    ...
    splitPlayerCell(client, parent, angle, mass, max) {
        ...
        if (mass) size2 = Math.sqrt(parent.radius - size1 * size1);
    ...
    spawnPlayer(client, position) {
        ...
        let index = ~~(this.nodesEject.length * Math.random()),
            eject = this.nodesEject[index];
        if (eject && eject.boostDistance < 1 && (Math.random((Math.random() * 100) + 0)) <= this.config.ejectSpawnChance) {
        ...
        for (let i = 0; i < 10 && this.willCollide(position, startSize); i++) position = this.randomPosition();
        let cell = new Entity.PlayerCell(this, client, position, startSize);
        this.addNode(cell);
        client.mouse = {
            x: position.x,
            y: position.y
    ...
    canEject(client) {
        ...
        let dt = this.tickCount - client.lastEject;
    ...
    loadConfig() {
        let config = this.src + "/config.ini",
    ...
    loadBadWords() {
        ...
        let badWordFile = this.src + "/badwords.txt";
        try {
    ...
    loadUserList() {
        ...
        let UserRoleEnum = require(this.src + "/enum/UserRoleEnum"),
    ...
    loadBanList() {
        let fileNameIpBan = this.src + "/ipbanlist.txt";
        try {
    ...
    startStats(port) {
        this.stats = "Test";
        this.getStats();
        this.httpServer = http.createServer(function(req, res) { // Cannot convert to arrow function
        ...
        this.httpServer.listen(port, function() { // Cannot convert to arrow function
