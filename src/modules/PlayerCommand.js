...
PlayerCommand.list = {
    help: function(args) {
        ...
        if (page === 2) {
            ...
            this.writeLine("/rec: Enables/disables supersplitter mode.");
    ...
    addbot: function(args) {
        ...
        var add = parseInt(args[1]); // args[1], 16
    ...
    explode: function() {
        ...
        for (var cell of client.cells) {
            for (;cell._size > this.gameServer.massToSize(10);) {
    ...
    kickbot: function(args) {
        ...
        var removed = 0;
        for (var client of this.gameServer.clients) {
    ...
    reset: function(args) {
        ...
        var gameServer = this.gameServer,
            ent = args[1];
        if (ent !== "all" && ent !== "ejected" && ent !== "food" && ent !== "virus" && ent !== "mothercell") return this.writeLine("[ERROR] Specify either 'food', 'virus', 'ejected', 'all', or 'mothercell'.");
        if (ent === "all") {
            this.writeLine("Removed " + gameServer.nodesAll.length + " entities.");
            for (;gameServer.nodesAll.length;) gameServer.removeNode(gameServer.nodesAll[0]);
            for (;gameServer.nodesEject.length;) gameServer.removeNode(gameServer.nodesEject[0]);
            for (;gameServer.nodesFood.length;) gameServer.removeNode(gameServer.nodesFood[0]);
            for (;gameServer.nodesVirus.length;) gameServer.removeNode(gameServer.nodesVirus[0]);
        ...
        if (ent === "ejected") {
            this.writeLine("Removed " + gameServer.nodesEject.length + " ejected cells.");
            for (;gameServer.nodesEject.length;) gameServer.removeNode(gameServer.nodesEject[0]);
        }
        if (ent === "food") {
            this.writeLine("Removed " + gameServer.nodesFood.length + " food cells.");
            for (;gameServer.nodesFood.length;) gameServer.removeNode(gameServer.nodesFood[0]);
        }
        if (ent === "virus") {
            this.writeLine("Removed " + gameServer.nodesVirus.length + " viruses.");
            for (;gameServer.nodesVirus.length;) gameServer.removeNode(gameServer.nodesVirus[0]);
        }
        if (ent === "mothercell") {
            if (gameServer.gameMode.ID !== 2) return this.writeLine("[ERROR] Mothercells can only be cleared in experimental mode.");
            this.writeLine("Removed " + gameServer.gameMode.mothercells.length + " mothercells.");
    ...
    eval: function(args) {
        if (!this.playerTracker.OP.enabled) return this.writeLine("[ERROR] You must have OP mode to use this command.");
        try {
            var string = args.slice(1, args.length).join(" ");
    ...
    /*,
    // TO DO: Fix command aliases (unsure how to outside of brute force copying commands).
    m: function(args) {
        PlayerCommand.list.mass(args, this);
    },
    sm: function(args) {
